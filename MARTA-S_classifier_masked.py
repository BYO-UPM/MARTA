...
def main(args, hyperparams):
    ...
    name = hyperparams["path_to_load"] + "/GMVAE_cnn_best_model_2d.pt"
    tmp = torch.load(name)
    model.load_state_dict(tmp["model_state_dict"])
    ...
    path = hyperparams["path_to_load"] + "/best_threshold.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="If 0, masks plosives, if 1, masks voiced plosives, if 2, masks nasals, if 3, masks fricatives, if 4, masks liquids, if 5, masks vowels, if 6 masks affricates, if 7 masks silences. Else, does not mask",
    )
    ...
    hyperparams = {
        ...
        "masked": args.masked,  # If 0, masks plosives, if 1, masks voiced plosives, if 2, masks nasals, if 3, masks fricatives, if 4, masks liquids, if 5, masks vowels, if 6 masks affricates, if 7 masks silences. Else, does not mask
        # ================ Training parameters ===================
