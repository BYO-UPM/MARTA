    ...
    for e in range(epochs):
        ...
        for batch_idx, (data, labels, g_label, r_label, b_label, manner, dataset) in enumerate(tqdm(trainloader)):
            ...
            if classifier:
                ...
                g_pred_list = np.concatenate((g_pred_list, \
                    [np.argmax(row) for row in y_pred[0].cpu().detach().numpy().squeeze()]))
                g_real_list = np.concatenate((g_real_list, g_label.cpu().detach().numpy()))
                r_pred_list = np.concatenate((r_pred_list, \
                    [np.argmax(row) for row in y_pred[1].cpu().detach().numpy().squeeze()]))
                r_real_list = np.concatenate((r_real_list, r_label.cpu().detach().numpy()))
                b_pred_list = np.concatenate((b_pred_list, \
                    [np.argmax(row) for row in y_pred[2].cpu().detach().numpy().squeeze()]))
