...
class GE2E_Loss(torch.nn.Module):
    ...
    def Estimate_cosine_sim(self,e,y):
        ...
        for i in range(batch):
            for k in classes:
                if k==y[i]:
                    S[i,k] = F.cosine_similarity(torch.unsqueeze(e[i,:],0), torch.unsqueeze((centroids[k]-e[i,:])/(n_samples[k] - 1),0))
                else:
                    S[i,k] = F.cosine_similarity(torch.unsqueeze(e[i,:],0), torch.unsqueeze(centroids[k]/n_samples[k],0))
        return S
    ...
    def embed_loss_contrast(self, cos_sim_matrix, y):
        ...
        for j in range(N):
            ...
            excl_centroids_sigmoids = torch.cat((sig_cos_sim_matrix[j,:y[j]], sig_cos_sim_matrix[j,y[j]+1:]))
            L.append(1. - centroids_sigmoids + torch.max(excl_centroids_sigmoids))
